...
const SwitchThumb = styled('span')(
  ...
  position: absolute;
  display: block;
  background-color: ${theme.palette.mode === 'dark' ? '#3a3a3a' : '#3a3a3a'};
  width: 32px;
  height: 32px;
  border-radius: 16px;
  top: 1px;
...
function MUISwitch(props) {
  ...
  return (
    <SwitchRoot onClick={switchHandler} className={clsx(stateClasses)}>
      <SwitchTrack>
...
export default function UseSwitchesCustom({ checked, darkHandler }) {
  return <MUISwitch checked={checked} darkHandler={darkHandler} />;
}